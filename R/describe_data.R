...
describe_data <- function(data, Y0 = "Y0", Y1 = "Y1", Z = "Z", sid = "sid") {
  ...
  data <- rename(data, Y0 = !!rlang::sym(Y0), Y1 = !!rlang::sym(Y1), Z = !!rlang::sym(Z), sid = !!rlang::sym(sid))
  sites <- data %>% group_by(!!rlang::sym(sid)) %>% summarise(Y0.bar = mean(Y0), Y1.bar = mean(Y1), beta = Y1.bar - Y0.bar, n = n(), p.Tx = mean(Z))
